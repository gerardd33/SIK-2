...
TcpClient::TcpClient(InputData& inputData)
	: inputData(inputData), socketDescriptor(-1) {
	establishConnection();
	setRadioTimeout();
...
void TcpClient::establishConnection() {
	...
	this->socketDescriptor =
		socket(addressInfo->ai_family, addressInfo->ai_socktype,
			   addressInfo->ai_protocol);
	if (socketDescriptor < 0) {
		ErrorHandler::syserr("socket");
	}

	if (connect(socketDescriptor, addressInfo->ai_addr, addressInfo->ai_addrlen) <
		0) {
...
addrinfo* TcpClient::getAddressInfo() {
	...
	if (getaddrinfo(this->inputData.getRadioHost(),
					this->inputData.getRadioPort(), &addressHints,
					&addressResult) != 0) {
